...
ALTER TABLE users RENAME TO customer1;
...
-----Basic Operation-----
-- Insert Single Customer1 Record
INSERT INTO customer1(first_name, last_name, email, created_date, updated_date, active)
	VALUES ('Foram','Babariya','foram.babariya@tatvasoft.com',now(),NULL,true);
...
SELECT * from orders
where customer_id IN (select customer_id from customer1 where active = true)
...
WHERE
	EXISTS (
		...
		FROM
			orders
		WHERE
			orders.customer_id = customer1.customer_id
	);
